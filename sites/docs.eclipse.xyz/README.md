---
domain: docs.eclipse.xyz
display_name: Eclipse Documentation
category: dev-tools
score: 59
grade: C
last_scored: 2026-05-27
verified_url: https://docs.eclipse.xyz/llms.txt
file_size_bytes: 22779
link_count: 143
value_link_count: 143
section_count: 3
freshness_age_days: null
---

# Eclipse Documentation

![Score 59/100 · Grade C](../../docs/badge/docs.eclipse.xyz.svg)

Category: **dev-tools** · [Live llms.txt](https://docs.eclipse.xyz/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** Eclipse Documentation


**File facts:** 22.2 KB · 3 `## sections` · 143 links (143 after low-value discount) · Freshness: unknown (no `Last-Modified` header).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 14/18 | _multi_h1_2, no_blockquote_summary_ |
| Coverage | 10/20 | _missing_canonical_sections_ |
| Agent-action declarations | 5/14 | _no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 5/10 | _no_last_modified_header_ |
| Discoverability | 8/8 | |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 5/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Discoverability (8/8)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Agent-action declarations (5/14): no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec
- Auth signposting (0/8): no_auth_keywords, no_auth_section

## Embed the badge

```markdown
[![llms.txt score 59 (C)](https://raw.githubusercontent.com/zriyansh/awesome-llms-txt/main/docs/badge/docs.eclipse.xyz.svg)](https://github.com/zriyansh/awesome-llms-txt/tree/main/sites/docs.eclipse.xyz)
```

## Reproduce this score

```bash
npx llms-txt-score https://docs.eclipse.xyz/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
