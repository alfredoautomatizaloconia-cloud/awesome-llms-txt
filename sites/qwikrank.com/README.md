---
domain: qwikrank.com
display_name: Qwikrank
category: infra
score: 58
grade: C
last_scored: 2026-05-27
verified_url: https://qwikrank.com/llms.txt
file_size_bytes: 1415
link_count: 9
value_link_count: 5
section_count: 3
freshness_age_days: 0
---

# Qwikrank

![Score 58/100 · Grade C](../../docs/badge/qwikrank.com.svg)

Category: **infra** · [Live llms.txt](https://qwikrank.com/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** Qwikrank: AI-Powered SEO Content Generation & Blog Hosting


**File facts:** 1.4 KB · 3 `## sections` · 9 links (5 after low-value discount) · Freshness: 0 days (per `Last-Modified`).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 16/18 | _no_blockquote_summary_ |
| Coverage | 6/20 | _few_value_links, missing_canonical_sections_ |
| Agent-action declarations | 0/14 | _no_llms_full_link, no_md_url_twins, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 10/10 | |
| Discoverability | 8/8 | |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 6/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Freshness (10/10)
- Discoverability (8/8)
- Size discipline (6/6)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Coverage (6/20): few_value_links, missing_canonical_sections
- Agent-action declarations (0/14): no_llms_full_link, no_md_url_twins, no_agent_signposts
- Auth signposting (0/8): no_auth_keywords, no_auth_section

## Embed the badge

```markdown
[![llms.txt score 58 (C)](https://raw.githubusercontent.com/zriyansh/awesome-llms-txt/main/docs/badge/qwikrank.com.svg)](https://github.com/zriyansh/awesome-llms-txt/tree/main/sites/qwikrank.com)
```

## Reproduce this score

```bash
npx llms-txt-score https://qwikrank.com/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
