---
domain: docs.1millionbot.com
display_name: 1millionbot
category: dev-tools
score: 58
grade: C
last_scored: 2026-05-27
verified_url: https://docs.1millionbot.com/llms.txt
file_size_bytes: 9930
link_count: 74
value_link_count: 74
section_count: 2
freshness_age_days: null
---

# 1millionbot

![Score 58/100 · Grade C](../../docs/badge/docs.1millionbot.com.svg)

Category: **dev-tools** · [Live llms.txt](https://docs.1millionbot.com/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** 1millionbot


**File facts:** 9.7 KB · 2 `## sections` · 74 links (74 after low-value discount) · Freshness: unknown (no `Last-Modified` header).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 14/18 | _multi_h1_2, no_blockquote_summary_ |
| Coverage | 9/20 | _thin_sectioning, missing_canonical_sections_ |
| Agent-action declarations | 5/14 | _no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 5/10 | _no_last_modified_header_ |
| Discoverability | 8/8 | |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 5/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Discoverability (8/8)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Agent-action declarations (5/14): no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec
- Auth signposting (0/8): no_auth_keywords, no_auth_section

## Embed the badge

```markdown
[![llms.txt score 58 (C)](https://raw.githubusercontent.com/zriyansh/awesome-llms-txt/main/docs/badge/docs.1millionbot.com.svg)](https://github.com/zriyansh/awesome-llms-txt/tree/main/sites/docs.1millionbot.com)
```

## Reproduce this score

```bash
npx llms-txt-score https://docs.1millionbot.com/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
