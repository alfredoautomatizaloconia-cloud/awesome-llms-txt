---
domain: agentskills.io
display_name: Agent Skills
category: dev-tools
score: 62
grade: C
last_scored: 2026-05-27
verified_url: https://agentskills.io/llms.txt
file_size_bytes: 1374
link_count: 9
value_link_count: 9
section_count: 1
freshness_age_days: 0
---

# Agent Skills

![Score 62/100 · Grade C](../../docs/badge/agentskills.io.svg)

Category: **dev-tools** · [Live llms.txt](https://agentskills.io/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** Agent Skills


**File facts:** 1.3 KB · 1 `## sections` · 9 links (9 after low-value discount) · Freshness: 0 days (per `Last-Modified`).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 15/18 | _no_blockquote_summary_ |
| Coverage | 5/20 | _thin_sectioning, few_value_links, missing_canonical_sections_ |
| Agent-action declarations | 6/14 | _no_llms_full_link, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 10/10 | |
| Discoverability | 8/8 | |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 6/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Freshness (10/10)
- Discoverability (8/8)
- Size discipline (6/6)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Coverage (5/20): thin_sectioning, few_value_links, missing_canonical_sections
- Auth signposting (0/8): no_auth_keywords, no_auth_section

## Embed the badge

```markdown
[![llms.txt score 62 (C)](https://raw.githubusercontent.com/zriyansh/awesome-llms-txt/main/docs/badge/agentskills.io.svg)](https://github.com/zriyansh/awesome-llms-txt/tree/main/sites/agentskills.io)
```

## Reproduce this score

```bash
npx llms-txt-score https://agentskills.io/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
