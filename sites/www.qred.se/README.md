---
domain: www.qred.se
display_name: Qred
category: dev-tools
score: 60
grade: C
last_scored: 2026-05-27
verified_url: https://www.qred.se/llms.txt
file_size_bytes: 7072
link_count: 38
value_link_count: 37
section_count: 10
freshness_age_days: 0
---

# Qred

![Score 60/100 · Grade C](../../docs/badge/www.qred.se.svg)

Category: **dev-tools** · [Live llms.txt](https://www.qred.se/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** Qred Bank llms.txt – Sverige


**File facts:** 6.9 KB · 10 `## sections` · 38 links (37 after low-value discount) · Freshness: 0 days (per `Last-Modified`).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 14/18 | _multi_h1_2, no_blockquote_summary_ |
| Coverage | 10/20 | _missing_canonical_sections_ |
| Agent-action declarations | 0/14 | _no_llms_full_link, no_md_url_twins, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 10/10 | |
| Discoverability | 8/8 | |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 6/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Freshness (10/10)
- Discoverability (8/8)
- Size discipline (6/6)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Agent-action declarations (0/14): no_llms_full_link, no_md_url_twins, no_agent_signposts
- Auth signposting (0/8): no_auth_keywords, no_auth_section

## Embed the badge

```markdown
[![llms.txt score 60 (C)](https://raw.githubusercontent.com/zriyansh/awesome-llms-txt/main/docs/badge/www.qred.se.svg)](https://github.com/zriyansh/awesome-llms-txt/tree/main/sites/www.qred.se)
```

## Reproduce this score

```bash
npx llms-txt-score https://www.qred.se/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
