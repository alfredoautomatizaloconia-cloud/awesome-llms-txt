---
domain: elevenlabs.io
display_name: ElevenLabs
category: ai-platform
score: 65
grade: B
last_scored: 2026-05-27
verified_url: https://elevenlabs.io/llms.txt
file_size_bytes: 9450
link_count: 47
value_link_count: 47
section_count: 7
freshness_age_days: null
---

# ElevenLabs

![Score 65/100 · Grade B](../../docs/badge/elevenlabs.io.svg)

Category: **ai-platform** · [Live llms.txt](https://elevenlabs.io/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** ElevenLabs


**File facts:** 9.2 KB · 7 `## sections` · 47 links (47 after low-value discount) · Freshness: unknown (no `Last-Modified` header).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 15/18 | _no_blockquote_summary_ |
| Coverage | 17/20 | |
| Agent-action declarations | 3/14 | _no_md_url_twins, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 5/10 | _no_last_modified_header_ |
| Discoverability | 8/8 | |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 5/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Discoverability (8/8)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Agent-action declarations (3/14): no_md_url_twins, no_agent_signposts, no_machine_readable_api_spec
- Auth signposting (0/8): no_auth_keywords, no_auth_section

## Embed the badge

```markdown
[![llms.txt score 65 (B)](https://raw.githubusercontent.com/zriyansh/awesome-llms-txt/main/docs/badge/elevenlabs.io.svg)](https://github.com/zriyansh/awesome-llms-txt/tree/main/sites/elevenlabs.io)
```

## Reproduce this score

```bash
npx llms-txt-score https://elevenlabs.io/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
