---
domain: docs.perplexity.ai
display_name: Perplexity
category: dev-tools
score: 77
grade: B+
last_scored: 2026-05-27
verified_url: https://docs.perplexity.ai/llms.txt
file_size_bytes: 29517
link_count: 145
value_link_count: 145
section_count: 3
freshness_age_days: 0
---

# Perplexity

![Score 77/100 · Grade B+](../../docs/badge/docs.perplexity.ai.svg)

Category: **dev-tools** · [Live llms.txt](https://docs.perplexity.ai/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** Perplexity

> Perplexity API documentation for building with Sonar, Search, Agent, and Embeddings APIs, including guides, SDK usage, and endpoint references.

**File facts:** 28.8 KB · 3 `## sections` · 145 links (145 after low-value discount) · Freshness: 0 days (per `Last-Modified`).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 18/18 | |
| Coverage | 12/20 | |
| Agent-action declarations | 10/14 | _no_llms_full_link_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 10/10 | |
| Discoverability | 8/8 | |
| Auth signposting | 2/8 | _no_auth_section_ |
| Size discipline | 5/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Spec compliance (18/18)
- Freshness (10/10)
- Discoverability (8/8)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Auth signposting (2/8): no_auth_section

## Embed the badge

```markdown
[![llms.txt score 77 (B+)](https://raw.githubusercontent.com/zriyansh/awesome-llms-txt/main/docs/badge/docs.perplexity.ai.svg)](https://github.com/zriyansh/awesome-llms-txt/tree/main/sites/docs.perplexity.ai)
```

## Reproduce this score

```bash
npx llms-txt-score https://docs.perplexity.ai/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
