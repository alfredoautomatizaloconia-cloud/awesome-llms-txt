---
domain: www.learninghub.cc
display_name: Learning Hub
category: dev-tools
score: 64
grade: C
last_scored: 2026-05-27
verified_url: https://www.learninghub.cc/llms.txt
file_size_bytes: 2246
link_count: 18
value_link_count: 18
section_count: 3
freshness_age_days: 0
---

# Learning Hub

![Score 64/100 · Grade C](../../docs/badge/www.learninghub.cc.svg)

Category: **dev-tools** · [Live llms.txt](https://www.learninghub.cc/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** Learning Hub

> AI-powered learning assistant that turns screen time into learning motivation. Tracks grades from school systems (EduPage, PRONOTE), checks homework using the Socratic method, assigns adaptive bonus tasks on weak topics, and manages a game minutes balance. Open-source, self-hoste…

**File facts:** 2.2 KB · 3 `## sections` · 18 links (18 after low-value discount) · Freshness: 0 days (per `Last-Modified`).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 18/18 | |
| Coverage | 9/20 | |
| Agent-action declarations | 1/14 | _no_llms_full_link, no_md_url_twins, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 10/10 | |
| Discoverability | 8/8 | |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 6/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Spec compliance (18/18)
- Freshness (10/10)
- Discoverability (8/8)
- Size discipline (6/6)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Agent-action declarations (1/14): no_llms_full_link, no_md_url_twins, no_machine_readable_api_spec
- Auth signposting (0/8): no_auth_keywords, no_auth_section

## Embed the badge

```markdown
[![llms.txt score 64 (C)](https://raw.githubusercontent.com/zriyansh/awesome-llms-txt/main/docs/badge/www.learninghub.cc.svg)](https://github.com/zriyansh/awesome-llms-txt/tree/main/sites/www.learninghub.cc)
```

## Reproduce this score

```bash
npx llms-txt-score https://www.learninghub.cc/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
