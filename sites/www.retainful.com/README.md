---
domain: www.retainful.com
display_name: Retainful
category: dev-tools
score: 53
grade: C
last_scored: 2026-05-27
verified_url: https://www.retainful.com/llms.txt
file_size_bytes: 8237
link_count: 31
value_link_count: 16
section_count: 8
freshness_age_days: null
---

# Retainful

![Score 53/100 · Grade C](../../docs/badge/www.retainful.com.svg)

Category: **dev-tools** · [Live llms.txt](https://www.retainful.com/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** llms.txt for Retainful.com


**File facts:** 8.0 KB · 8 `## sections` · 31 links (16 after low-value discount) · Freshness: unknown (no `Last-Modified` header).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 11/18 | _multi_h1_14, no_blockquote_summary, sections_without_link_lists, malformed_link_items_ |
| Coverage | 13/20 | |
| Agent-action declarations | 0/14 | _no_llms_full_link, no_md_url_twins, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 5/10 | _no_last_modified_header_ |
| Discoverability | 8/8 | |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 5/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 1/2 | _marketing_fluff_ |

## What's exceptional

- Discoverability (8/8)
- Content-Type & encoding (4/4)

## What's weak

- Agent-action declarations (0/14): no_llms_full_link, no_md_url_twins, no_agent_signposts
- Auth signposting (0/8): no_auth_keywords, no_auth_section

## Embed the badge

```markdown
[![llms.txt score 53 (C)](https://raw.githubusercontent.com/zriyansh/awesome-llms-txt/main/docs/badge/www.retainful.com.svg)](https://github.com/zriyansh/awesome-llms-txt/tree/main/sites/www.retainful.com)
```

## Reproduce this score

```bash
npx llms-txt-score https://www.retainful.com/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
