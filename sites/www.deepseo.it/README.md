---
domain: www.deepseo.it
display_name: DeepSEO Corso SEO Online
category: dev-tools
score: 54
grade: C
last_scored: 2026-05-27
verified_url: https://www.deepseo.it/llms.txt
file_size_bytes: 50350
link_count: 104
value_link_count: 104
section_count: 3
freshness_age_days: null
---

# DeepSEO Corso SEO Online

![Score 54/100 · Grade C](../../docs/badge/www.deepseo.it.svg)

Category: **dev-tools** · [Live llms.txt](https://www.deepseo.it/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** DeepSEO Corso SEO Online


**File facts:** 49.2 KB · 3 `## sections` · 104 links (104 after low-value discount) · Freshness: unknown (no `Last-Modified` header).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 15/18 | _no_blockquote_summary, sections_without_link_lists_ |
| Coverage | 10/20 | _missing_canonical_sections_ |
| Agent-action declarations | 0/14 | _no_llms_full_link, no_md_url_twins, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 5/10 | _no_last_modified_header_ |
| Discoverability | 8/8 | |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 4/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Discoverability (8/8)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Agent-action declarations (0/14): no_llms_full_link, no_md_url_twins, no_agent_signposts
- Auth signposting (0/8): no_auth_keywords, no_auth_section

## Embed the badge

```markdown
[![llms.txt score 54 (C)](https://raw.githubusercontent.com/zriyansh/awesome-llms-txt/main/docs/badge/www.deepseo.it.svg)](https://github.com/zriyansh/awesome-llms-txt/tree/main/sites/www.deepseo.it)
```

## Reproduce this score

```bash
npx llms-txt-score https://www.deepseo.it/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
