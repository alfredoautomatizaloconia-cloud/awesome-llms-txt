---
domain: docspring.com
display_name: DocSpring Integration Guide
category: dev-tools
score: 50
grade: C
last_scored: 2026-05-27
verified_url: https://docspring.com/llms.txt
file_size_bytes: 4445
link_count: 0
value_link_count: 0
section_count: 5
freshness_age_days: 12
---

# DocSpring Integration Guide

![Score 50/100 · Grade C](../../docs/badge/docspring.com.svg)

Category: **dev-tools** · [Live llms.txt](https://docspring.com/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** DocSpring Integration Guide


**File facts:** 4.3 KB · 5 `## sections` · 0 links (0 after low-value discount) · Freshness: 12 days (per `Last-Modified`).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 9/18 | _no_blockquote_summary, sections_without_link_lists, malformed_link_items_ |
| Coverage | 5/20 | _few_value_links_ |
| Agent-action declarations | 0/14 | _no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 9/10 | |
| Discoverability | 8/8 | |
| Auth signposting | 2/8 | _no_auth_section_ |
| Size discipline | 6/6 | |
| Content-Type & encoding | 3/4 | _no_utf8_charset_ |
| Voice | 2/2 | |

## What's exceptional

- Discoverability (8/8)
- Size discipline (6/6)
- Voice (2/2)

## What's weak

- Coverage (5/20): few_value_links
- Agent-action declarations (0/14): no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec
- Auth signposting (2/8): no_auth_section

## Embed the badge

```markdown
[![llms.txt score 50 (C)](https://raw.githubusercontent.com/zriyansh/awesome-llms-txt/main/docs/badge/docspring.com.svg)](https://github.com/zriyansh/awesome-llms-txt/tree/main/sites/docspring.com)
```

## Reproduce this score

```bash
npx llms-txt-score https://docspring.com/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
